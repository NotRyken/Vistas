...
public class PanoramaResourceReloader extends SimplePreparableReloadListener<HashMap<Identifier, Pair<PanoramaControl, List<String>>>> {
    ...
    @Override
    protected @NotNull HashMap<Identifier, Pair<PanoramaControl, List<String>>> prepare(
            ResourceManager manager,
            ProfilerFiller profiler
    ) {
        profiler.startTick();
        HashMap<Identifier, PanoramaControl> panoramaCtrls = Maps.newHashMap();
                        ...
                        try {
                            ...
                            try {
                                ...
                                jsonElement.getAsJsonObject().entrySet().forEach((pair) -> {
                                    ...
                                    PanoramaControl panoramaCtrl = get(PanoramaControl.CODEC, pair.getValue());
                                    if (panoramaCtrl != null) {
                                        panoramaCtrls.put(panoramaId, panoramaCtrl);
                                        Pair<List<String>, List<Identifier>> splashes = prepare(panoramaCtrl.splashText(), manager, profiler);
                                        web.put(panoramaId, splashes);
                                        if (!parsedSplashWeb.containsKey(panoramaCtrl.splashText())) {
                                            parsedSplashWeb.put(panoramaCtrl.splashText(), splashes);
                                        }
                                    } else {
        ...
        HashMap<Identifier, Pair<PanoramaControl, List<String>>> panoramaMap = Maps.newHashMap();
        panoramaCtrls.forEach((panoramaId, panorama) -> panoramaMap.put(panoramaId, Pair.of(panorama, this.splashTexts.get(panoramaId))));
        profiler.endTick();
        return panoramaMap;
    }

    @SuppressWarnings("unused")
    protected Pair<List<String>, List<Identifier>> prepare(
            Identifier splashId,
            ResourceManager manager,
            ProfilerFiller profiler
    ) {
    ...
    protected void iterateImports(
            Identifier panoramaId,
            List<Identifier> imports,
            List<Identifier> seenImports,
            List<String> addTo
    ) {
    ...
    protected void apply(
            HashMap<Identifier, Pair<PanoramaControl, List<String>>> prepared,
        ...
        prepared.forEach((panoramaId, pair) -> {
            VistasTitle.registerPanorama(panoramaId, pair.getFirst());
            this.splashTexts.put(panoramaId, pair.getSecond());
        });

        profiler.push("builtin");
        VistasTitle.MOD_PANORAMAS.forEach(VistasTitle::registerPanorama);
        profiler.pop();

        profiler.pop();

        if (VistasConfig.getInstance().randomPerScreen || VistasConfig.getInstance().forcePanorama) {
            VistasTitle.selectPanorama(profiler);
        } else {
